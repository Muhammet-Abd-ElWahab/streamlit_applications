...
def disply_test_train(test_train, name):
    ...
    # Configure columns with specific settings but keep original colors
    gd.configure_column("Test_Score", header_name="Test_Score", filter=True,Width=20,
                    cellStyle={'textAlign': 'left', 'fontSize': '16px', 'fontFamily': 'Arial, sans-serif', 'color': 'red'},
                    headerClass="left-header")
    
    gd.configure_column("Train_Score", header_name="Train_Score", filter=True,
                    cellStyle={'textAlign': 'left', 'fontSize': '16px', 'fontFamily': 'Arial, sans-serif', 'color': '#008080'},
                    headerClass="left-header")
    
    gd.configure_column("Metric", header_name="Metric", filter=True,
                cellStyle={'textAlign': 'left', 'fontSize': '16px', 'fontFamily': 'Arial, sans-serif', 'color': '#008080'},
                headerClass="left-header")
    ...
    # Set grid options for full width but left alignment
    ...
    # Display the grid
    grid_table = AgGrid(test_train.reset_index(),
...
def disply_scores(df_scores, name):
    ...
    gd.configure_column("Score", header_name="Score", filter=True,minWidth=20,
                    cellStyle={'textAlign': 'left', 'fontSize': '16px', 'fontFamily': 'Arial, sans-serif', 'color': '#008080'},
                    headerClass="left-header")
    gd.configure_column("Interpretation", header_name="Interpretation", filter=True, width=600 , flex=7,
                    cellStyle={'textAlign': 'left', 'fontSize': '16px', 'fontFamily': 'Arial, sans-serif', 'color': '#008080'},
                    headerClass="left-header")
    
    gd.configure_column("Metric", header_name="Metric", filter=True,minWidth=20,
                    cellStyle={'textAlign': 'left', 'fontSize': '16px', 'fontFamily': 'Arial, sans-serif', 'color': '#008080'},
                    headerClass="left-header")
    ...
    # Set grid options for full width and left alignment
    gridoptions = gd.build()
    gridoptions['domLayout'] = 'normal'
    ...
    # Display the grid
    grid_table = AgGrid(df_scores.reset_index(),
                gridOptions=gridoptions,
                update_mode=GridUpdateMode.MODEL_CHANGED | GridUpdateMode.SELECTION_CHANGED,
                height=290,
                allow_unsafe_jscode=True,
...
if __name__ == "__main__":
    ...
    if model_select == "XGBoost":
        ...
        all_scores_df2, train_test_scores_df2, selected_model, ex_time = ml_model("lr")
