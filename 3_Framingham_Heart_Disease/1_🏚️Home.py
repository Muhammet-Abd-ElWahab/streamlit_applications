...
from functions import df_clean, bar_chart
...
def chd_proptions():
    TenY_propotions = df_clean['TenYearCHD'].value_counts(normalize=True)
    TenY_propotions = pd.DataFrame(TenY_propotions).reset_index()
    TenY_propotions["TenYearCHD"] = TenY_propotions["TenYearCHD"].apply(lambda x: "CHD" if x ==1 else "No CHD")
    TenY_propotions["proportion"] = round(TenY_propotions["proportion"]*100, 0)
    TenY_propotions["proportion"] = TenY_propotions["proportion"].astype(int)
    TenY_propotions["count"] = [i for i in df_clean['TenYearCHD'].value_counts().values]

    fig = px.pie(TenY_propotions, values='proportion', names="TenYearCHD",
            labels = {'proportion': 'Percentage',},
            custom_data=['count'], 
             hover_data={'TenYearCHD': False,'count':True},
             color_discrete_sequence=['#008294','#bdbdbd'],height=300)
    ...
    fig.update_layout(
        title={'text': 'Proportion of Patients with Ten-Year CHD Risk', 'y': 0.95, 'x': 0.5, 'xanchor': 'center', 'yanchor': 'top'},  # Center and adjust title
        title_font_size=20,  # Title font size
        legend_title_text='Risk',  # Legend title
